...
def test_three_bases(self):
    b1 = Base()
    """ b2 = Base()  Remove or comment out unused variable"""
    b3 = Base()
...
def test_nb_instances_after_unique_id(self):
    b1 = Base()
    """ b2 = Base(12) Remove or comment out unused variable"""
    b3 = Base()
...
def test_invalid_id_types(self):
    invalid_ids = ["hello", 5.5, complex(5), {"a": 1, "b": 2}, True,
                   [1, 2, 3], (1, 2), {1, 2, 3}, frozenset({1, 2, 3}),
                   range(5),
                   (b'Python'), bytearray(b'abcefg'), memoryview(b'abcefg'),
                   float('inf')]
    for invalid_id in invalid_ids:
        with self.subTest(invalid_id=invalid_id):
            self.assertEqual(invalid_id, Base(invalid_id).id)


def test_invalid_id_values(self):
    invalid_ids = [float('nan')]
    for invalid_id in invalid_ids:
        with self.subTest(invalid_id=invalid_id):
            self.assertNotEqual(invalid_id, Base(invalid_id).id)
